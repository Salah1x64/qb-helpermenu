...
RegisterNetEvent('qb-helper:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('qb-helper:client:spectate', function(targetPed, coords)
...
RegisterNetEvent('qb-helper:client:SendReport', function(name, src, msg)
    TriggerServerEvent('qb-helper:server:SendReport', name, src, msg)
end)

RegisterNetEvent('qb-helper:client:SendStaffChat', function(name, msg)
    TriggerServerEvent('qb-helper:server:Staffchat:addMessage', name, msg)
end)
...
RegisterNetEvent('qb-helper:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('qb-helper:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('qb-helper:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('qb-helper:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('qb-helper:client:GiveNuiFocus', function(focus, mouse)

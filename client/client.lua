...
RegisterNetEvent('qb-helper:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
RegisterNetEvent('qb-helper:client:ToggleCoords', function()
...
local function OpenPermsMenu(permsply)
    QBCore.Functions.TriggerCallback('qb-helper:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('qb-helper:server:setPermissions', permsply.id, selectedgroup)
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('qb-helper:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('qb-helper:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('qb-helper:server:'..values, player)
